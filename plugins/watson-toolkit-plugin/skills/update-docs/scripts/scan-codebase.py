...
# ///
"""
Codebase Scanner — bundled from Cartographer (MIT License)
Original: https://github.com/kingbootoshi/cartographer
Copyright (c) 2025 Bootoshi
...
# Note: names like ".env", "build", "dist" match both directories AND files with that name.
DEFAULT_IGNORE = {
    # Directories (also matches files with the same name)
    ".git",
    ".svn",
    ".hg",
    "node_modules",
...
def format_tree(scan_result: dict, show_tokens: bool = True) -> str:
    ...
    def print_tree(node: dict, prefix: str = ""):
        # Sort directories (dicts without 'tokens') before files (dicts with 'tokens'), then alphabetically
