...
```
# Writer: every hour
0 * * * *   [your agent] skill run random-thought --stage writer

# Curator: daily at a quiet hour
0 5 * * *   [your agent] skill run random-thought --stage curator
```
...
- **History file path drift** — If files move or are renamed, the freshness gate won't recognize them as visited and may re-select them before the cooldown expires.
- **Large workspaces** — The corpus scanner uses `find`. Workspaces with 100k+ files may have slow first runs. Use `excludePatterns` aggressively.
- **macOS sort compatibility** — `sort -R` is not available on macOS. `corpus-pick.sh` falls back to a Python shuffle — requires `python3` on PATH.
- **Curator requires prior Writer runs** — If `output.dir` is empty (no prior Writer invocations), the Curator will produce an empty digest. Run the Writer stage first.
- **Same file re-selected** — If `freshness.enabled` is false or the history file is missing, the same file can be selected on consecutive runs. Verify config and history file path.
- **Output quality is model-dependent** — The Writer stage is only as good as the model running it. Better models produce better reflections; small models tend toward generic summaries.

## Dependencies

- `bash` — Required for corpus-pick.sh and freshness-gate.sh
- `python3` — Required for macOS-compatible random shuffle in corpus-pick.sh (fallback when `sort -R` unavailable)
- `find` — Standard on macOS and Linux; required for corpus scanning
- No npm/pip packages — zero runtime dependencies beyond the above
